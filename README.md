...
Let's break down the command line above. It uses SHELL=pmash to force
make to use the auditor as a shell wrapper and .SHELLFLAGS to control
the flags passed to it; in particular pmash takes the usual -c string
and passes it to the shell. It adds "-o $@.d" to send derived prereq
data to foo.d when building foo.
